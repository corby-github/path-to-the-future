...
| `east-corridor` | East corridor | universal | medium | Top + bottom frame walls compress player into a 140-px east-west corridor (y=230..370) containing spawn + door. Horizontal patrol sweeps the corridor at y=275..315, leaving 45-px safe band above and 55-px below. Period 2.8 s. v2.0.25. |
...
| `triangle-sentinel` | Triangle sentinel | universal | medium | Top + bottom frame walls compress player into y=130..470 corridor; single sentinel traces a triangular path (top-left → top-right → bottom-center → loop). Period 4.5 s = 1.5 s per segment. Three safe corners the sentinel isn't currently at. v2.0.25. |
| `sync-patrols` | Sync patrols | universal | medium | Center wall (x=470..530, y=240..360) forces detour into one of two patrol bands; both patrols move in SYNC (same phase, same direction) at y=200 and y=400, period 2.8 s each. Differs from `twin-patrols` (counter-direction) — readable wave instead of timing puzzle. v2.0.25. |
