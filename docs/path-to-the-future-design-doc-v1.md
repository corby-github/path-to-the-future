...
**Current pool (35 templates — 13 simple + 4 easy + 8 medium + 7 hard + 3 expert as of v2.0.24):**
...
| `crossfire` | Crossfire | universal | hard | Eight motions in the right half: 4 horizontal sweepers at y=100/180/380/460 (periods 2500–2800, alternating start directions), 3 vertical pendulums at x=560/680/810 (periods 1700–1900, phase-staggered), + 1 door paddle at x=910 (period 1500). Non-aligned periods across all 8 so the combined hazard pattern never visibly repeats. v2.0.24 — initial 3-motion drop read as "good, hard, not expert unless you added more pickets"; amped to 8 motions per user request to test for expert tier. Tier currently `hard` — flip to `expert` if playtest confirms feel. |
...
- SWE simple = 12 (universal 9 + swe 3); SWE easy = 4 (universal); SWE medium = 8 (universal); SWE hard = 7 (universal); SWE expert = 3 (universal); SWE total eligible = 34.
- Homeschool simple = 10 (universal 9 + homeschool 1); Homeschool easy = 4 (universal); Homeschool medium = 8 (universal); Homeschool hard = 7 (universal); Homeschool expert = 3 (universal); Homeschool total eligible = 32.

In 2020 (100% simple) SWE picks from 12 / Homeschool from 10. From 2021 onwards the easy pool engages at the v2.0.9 mix weights. From 2023 onwards medium rolls pull from the eight medium templates rather than falling through to easy. From 2027 onwards hard rolls pull from the seven hard templates. From 2029 onwards expert rolls pull from the three expert templates rather than falling through to hard.
