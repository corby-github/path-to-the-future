...
| v2.0.23 | 2026-05-15 | Corby Hoback · Claude Code | **Medium-tier expansion (issue #94, batch 1) + tuning pass.** Six new medium templates composing existing patterns (maze geometry + motion): `gate-paddle`, `s-curve-patrol`, `switchback-paddle`, `slow-orbit` (renamed *Corralled orbit*), `twin-patrols`, `maze-gauntlet`. Initial drop read as easy; tuning pass lifted periods to medium tempo (2400–3500 ms range vs `pendulum` / `shutters` at 2400) and added forcing geometry (corridor frames, center walls, paddle pairs) so each template demands timing instead of allowing route-around. Pure content authoring on the v2.0.22 engine. Medium pool 2→8 closes most of the 2023–2026 repetition gap. See §4 *Layout templates* (six new medium rows) + pool-size update. |
...
**Current pool (34 templates — 11 simple + 4 easy + 13 medium + 4 hard + 2 expert as of v2.0.25):**
...
**Pool sizes per pack (v2.0.25):**
- SWE simple = 10 (universal 7 + swe 3); SWE easy = 4 (universal); SWE medium = 13 (universal); SWE hard = 4 (universal); SWE expert = 2 (universal); SWE total eligible = 33.
- Homeschool simple = 8 (universal 7 + homeschool 1); Homeschool easy = 4 (universal); Homeschool medium = 13 (universal); Homeschool hard = 4 (universal); Homeschool expert = 2 (universal); Homeschool total eligible = 31.

In 2020 (100% simple) SWE picks from 10 / Homeschool from 8. From 2021 onwards the easy pool engages at the v2.0.9 mix weights. From 2023 onwards medium rolls pull from the thirteen medium templates rather than falling through to easy. From 2027 onwards hard rolls actually pull the four hard templates. From 2029 onwards expert rolls actually pull `zigzag-sentinel` / `patrol-and-paddle` rather than falling through to hard.
