...
**Document version:** 2.0.32
**Status:** Living spec · Days 1–14 (title screen) merged · Day 15 — analytics wrapper + slug/event instrumentation wired (GoatCounter `pathtothefuture.goatcounter.com`); GitHub Pages deploy still pending · Two packs playable (SWE + Homeschool Parent) · Half-length playthrough (1 cinematic + 6 playable months/year, 70 monthIds total) · 35 layout templates (13 simple + 4 easy + 8 medium + 6 hard + 4 expert) · Room complexity tier ladder PR4/5/6 — moving-obstacle physics + medium + hard + expert tiers populated · All 8 class tiers selectable · Finale trophy on the recap screen · Kid names player-controlled in Homeschool pack · NPC palette tokens split out from `accent` (adult vs child) · Replay spawn obstacle-aware (wall-stuck-on-re-entry guard) · Keyboard tutorial widget + double-tap-to-sprint (issues #89, #92)
...
|---------|------------|---------------------------|---------|
| v2.0.33 | 2026-05-16 | Corby Hoback · Claude Code | **Expert vertical obstacles — touch top + bottom (no cruise lanes).** Per playtest direction: `tempest` and `crossfire` both had vertical pendulums + door paddles that left cruise lanes above and below the sweep range, letting the player route past the timing puzzle. Fix is geometric: set `baseRect.y = amp = (600 − height) / 2` so each body touches both playable edges at extremes. **Tempest:** 4 pendulums (h=80) baseRect.y stayed at 260, amp 180 → 260 (body sweeps y=0..80 at top extreme, y=520..600 at bottom). Door paddle (h=80) amp 180 → 260. **Crossfire:** 3 pendulums (h=100) baseRect.y normalized to 250 (was 300/320/300 — staggered for visual variety, now uniform for symmetric reach), amp 150 → 250 (body sweeps y=0..100 at top, y=500..600 at bottom). Door paddle (h=80) amp 180 → 260. No new templates, no engine change — pure parameter retune on existing expert pair. |
| v2.0.32 | 2026-05-16 | Corby Hoback · Claude Code | **Tutorial restructure — intro step + inline sprint step.** Per playtest direction: (1) Add a `Quick tutorial` intro step at index 0 ("I'll show you how to play. Any key to continue.") — centered, advances on any key. (2) Move the sprint teach inline as step 3, right after the keys-widget step — was previously a separate `SprintTutorialOverlay` that fired after 5 s of cumulative baseline movement. New flow is fully sequential: intro → status bar → movement → sprint → objects & people → door (6 steps). Sprint step renders the `[→] + [→] Go fast right` example row inline. `SprintTutorialOverlay` component deleted; `meta.sprintTutorialDismissed` flag + `dismissSprintTutorial` action removed (single `tutorialDismissed` covers everything again); cumulative-movement timer in `DecisionRoom.handleTick` removed. `TUTORIAL_KEYS_STEP_INDEX` bumped 1 → 2 to reflect the new positioning. No `STATE_VERSION` bump (the removed flag was always read with `?? false`; old saves with it set are simply ignored). |
| v2.0.31 | 2026-05-16 | Corby Hoback · Claude Code | **HUD tier label — at-a-glance playtest aid.** `CurrentRoomContext` widens with `tier: string \| null` alongside `template`; `setTemplate(id)` replaced with `setRoomInfo({ template, tier })`. DecisionRoom publishes `layout.complexity` on mount, clears on unmount. HUD's location column renders the tier as a small uppercase tag after the template name (e.g. `open-office · SIMPLE`). 1 px `palette.surface` border + `palette.ink` text → readable across era-mood palette shifts. Non-decision rooms (Narrative / Minigame / Consequence) don't publish, so the tag is absent there. No engine / state-shape changes. |
| v2.0.30 | 2026-05-16 | Corby Hoback · Claude Code | **Keyboard tutorial widget + double-tap-to-sprint (issues #89, #92).** The game is keyboard-only but no surface taught that — a player who clicks the canvas got no feedback. This PR teaches keyboard movement in two surfaces and adds a sprint upgrade. (1) New `KeysWidget` component (`src/game/ui/KeysWidget.tsx`) — stateless SVG with two cross/diamond clusters (arrows + WASD). (2) First-run tutorial gains a "Move with the keyboard" step at index 1 (4-step coachmark, was 3). (3) New `SprintTutorialOverlay` fires after the main coachmark dismisses AND the player accumulates ~5 s of baseline movement — single step, pulses the player's most-used direction in the widget. (4) New `useMisclickPrompt` hook surfaces the keys widget centered on the canvas whenever the player clicks it (auto-dismisses 3 s OR on next movement keypress; suppressed during modals / tutorial / keys-widget step). (5) Double-tap-to-sprint: `useKeyboardInput` detects a second keydown within 280 ms of the prior keyup as `sprintAxis`; OS key-repeat is filtered (`lastKeyDownAt > lastKeyUpAt` means held, not tapped). `usePlayerMovement` applies `SPRINT_MULTIPLIER = 2.0` to the matching axis post-normalization. (6) Motion lines render behind the player while `sprintingAxis` is set (3 short stroke segments at 40 % `palette.ink` opacity, anchored to player, trailing opposite the travel direction). (7) Knockback suppresses sprint by virtue of the `if (ext)` branch never touching the sprint path. (8) Minigames isolated — each uses its own `window.addEventListener('keydown')` and doesn't import `useKeyboardInput`, so sprint never activates inside them. (9) New `meta.sprintTutorialDismissed` flag (back-compat: `?? false` on read; `resetTutorial` clears both). No `STATE_VERSION` bump. |
| v2.0.29 | 2026-05-16 | Corby Hoback · Claude Code | **Medium playtest pass on PR #97 batch (geometric read).** Three of the six v2.0.25 medium templates had cruise lanes that bypassed their motion — same failure mode as PR #95's `s-curve-patrol` and PR #96's `counter-patrols`/`channel-paddle` ("open geometry + thin motion = plays easier than intended"). Fixes: (1) `east-corridor` — corridor narrowed from 140 px (y=230..370) to 80 px (y=260..340) via thicker top + bottom frames; the wider corridor allowed `dy²>radius²` clearance from patrol body at y=244..261 and y=329..356. (2) `triangle-sentinel` — period 4500 → 3000 ms (1000 ms/segment, was 1500) to close the ~3.7 s window when the single sentinel was at a corner away from the natural y=300 cruise line. (3) `sync-patrols` — added top + bottom frame walls (y=0..170, y=430..600) to close the cruise zones above/below the patrol bands; player now MUST traverse through one of the two patrol bands (center wall pushes them in either way). The other three (`triple-paddle-slow`, `asymmetric-block`, `mini-orbits`) verified solid — no changes. Geometric analysis only — physical playtest still recommended to confirm the fixes land. See §4 *Layout templates*. |
| v2.0.28 | 2026-05-16 | Corby Hoback · Claude Code | **Expert tier recast — crossfire is the only true expert.** Per playtest direction: 4 templates at expert was over-supplied; only `crossfire` (8 motions, mixed-axis density, non-aligned periods, no permanent safe zone) was reading as expert. (1) Demoted `tight-pickets`, `zigzag-sentinel`, `patrol-and-paddle` expert → hard with rationale comments in `layouts.ts`. (2) New `tempest` expert template — 7 motions in the right half (2H path-sweeps at top + bottom limiting cruise lanes, 4V pendulums spaced 100 px apart phase-staggered π/2 with periods 1600/1800/2000/2200, 1 door-front paddle period 1500). All periods distinct (1500/1600/1800/2000/2200/2400/2700). Differs from crossfire by denser-vertical / lighter-horizontal motion mix and tighter pendulum spacing. (3) Net tier shifts: −3 expert, +3 hard, +1 expert new = hard pool 6 → 9, expert pool 4 → 2. No engine change. See §4 *Layout templates*. |
...
| `triple-paddle-slow` | Triple paddle (slow) | universal | medium | Three paddles in a row (x=400/600/800), 120° phase-staggered, amp 220, period 2.8 s. Medium-tempo cousin of the hard `triple-paddle`. Amp 220 makes pickets touch close to top + bottom playable edges (no cruise-along-edge lane). v2.0.25. |
| `east-corridor` | East corridor | universal | medium | Top + bottom frame walls compress player into an 80-px east-west corridor (y=260..340) containing spawn + door. Horizontal patrol sweeps the corridor at y=275..315. v2.0.25 → v2.0.29 — corridor narrowed from 140 px to 80 px after geometric read found cruise lanes at y=244..261 + y=329..356 (`dy²>radius²` from patrol body). Period 2.8 s. |
| `asymmetric-block` | Asymmetric block | universal | medium | Large 180×300 block on the left half (x=300..480, y=150..450) forces detour above (y<150) or below (y>450), then a vertical paddle at x=750 (amp 150, period 2.8 s) sweeps a 240-px range so the door approach demands timing regardless of route. v2.0.25. |
| `triangle-sentinel` | Triangle sentinel | universal | medium | Top + bottom frame walls compress player into y=130..470 corridor; single sentinel traces a triangular path (top-left → top-right → bottom-center → loop). Period 3.0 s = 1.0 s per segment. Three safe corners the sentinel isn't currently at. v2.0.25 → v2.0.29 — period 4.5 s → 3.0 s after geometric read found the slow tempo created ~3.7 s clear windows at y=300 between corner crossings. |
| `sync-patrols` | Sync patrols | universal | medium | Top + bottom frame walls + center wall (x=470..530, y=240..360) force the player into one of two patrol bands (upper y=170..240 or lower y=360..430); both patrols move in SYNC (same phase, same direction) at y=200 and y=400, period 2.8 s each. Differs from `twin-patrols` (counter-direction) — readable wave instead of timing puzzle. v2.0.25 → v2.0.29 — added top + bottom frames (y=0..170, y=430..600) after geometric read found cruise lanes above/below the patrol bands. |
| `mini-orbits` | Mini orbits | universal | medium | Top + bottom frame walls compress player into y=130..470 corridor; TWO small sentinel orbits at offset x-ranges and offset y-bands (upper-middle x=280..520 y=180..280, lower-middle x=440..680 y=320..420). Orbits overlap in x but not y, so player threads between them. Period 3.5 s each. v2.0.25. |
...
| `paddle-pair-phase` | Paddle pair (phase) | universal | medium | Two paddles 140 px apart in front of door, phase-offset π (one up while other down). Period 2 s. v2.0.24 — initially shipped hard; playtest read it as simple (open geometry, single beat), reclassified to medium. |
| `counter-patrols` | Counter patrols | universal | simple | Top + bottom static walls + two counter-direction patrols at y=200 / y=380. Period 2.4 s. v2.0.24 — initially shipped hard; playtest read it as easy/simple (155 px safe middle between the patrols), reclassified to simple. |
| `channel-paddle` | Channel paddle | universal | simple | Static walls form a 160-px vertical channel + a paddle inside. v2.0.24 — initially shipped hard; playtest read it as casual (single-paddle timing, paddle never fully closes the channel), reclassified to simple. |
| `tight-pickets` | Tight pickets | universal | hard | 5 narrow oscillators packed tight at x=510..870 (~90 px center-to-center, all right-half), period 1.5 s, 72° phase wave. v2.0.24 — amplitude bumped 180 → 210 so pickets touch close to top + bottom playable edges at extremes. v2.0.28 — re-demoted expert → hard per playtest direction "crossfire is the only true expert" — tight-pickets is single-axis with uniform period; reads as a tempo wall rather than the puzzle-storm character that defines expert. |
| `triple-paddle` | Triple paddle | universal | hard | Three paddles in a row in front of door, 120° phase-staggered. v2.0.24 — amplitude bumped 220 → 230 per "make sure the pickets go close to top and bottom". Period 1.8 s. |
| `crossfire` | Crossfire | universal | expert | Eight motions in the right half: 4 horizontal sweepers at y=100/180/380/460 (periods 2500–2800, alternating start directions), 3 vertical pendulums at x=560/680/810 (periods 1700–1900, phase-staggered), + 1 door paddle at x=910 (period 1500). Non-aligned periods across all 8 so the combined hazard pattern never visibly repeats. v2.0.24 — initial 3-motion drop read as hard ("not expert unless you added more pickets"); amped to 8 motions, playtest confirmed expert feel → reclassified hard → expert. |
| `gauntlet` | Gauntlet | universal | hard | v2.0.24 — full redesign after v1 played as WAY too easy. Three snake walls each blocking a full half-canvas (200-px gaps, was 310 px), plus three phase-offset paddles (one per corridor the player traverses), 120° phase stagger, period 1.5 s. Three sequential timing windows instead of one weak central beat. |
| `zigzag-sentinel` | Zigzag sentinel | universal | hard | First template using the v2.0.22 deterministic-path motion: 40×40 sentinel traces a bowtie loop through the 4 corners of the right half (period 8 s, ~2 s per segment). Slow + readable; the middle y-band is dead-air safe zone. v2.0.28 — demoted expert → hard: the permanent middle safe zone (a single deterministic sentinel can't block it) doesn't match expert's "no permanent safe zone" character. |
| `patrol-and-paddle` | Patrol & paddle | universal | hard | Mixes motion modes ("medium/hard, plus a deterministic block"): 80×25 horizontal patrol on a path (x=500..880 at y=280, period 4 s) + a 20×80 sine paddle near the door (y=80..440, period 1.8 s). Non-integer-ratio periods so the combined pattern doesn't repeat. v2.0.28 — demoted expert → hard: two-motion density is standard hard, not expert. |
| `tempest` | Tempest | universal | expert | v2.0.28 — second expert template, matching crossfire's character recipe (mixed-axis density + non-aligned periods + no permanent safe zone) with a different motion mix: 2 horizontal sweepers at top + bottom (path-based, opposite directions, periods 2.4 / 2.7 s) + 4 vertical pendulums spaced 100 px apart in the right half (x=540/640/740/840, π/2 phase-staggered, periods 1.6 / 1.8 / 2.0 / 2.2 s, amp 180) + 1 door-front paddle (x=910, period 1.5 s). 7 motions, all-distinct periods (1500/1600/1800/2000/2200/2400/2700) so the combined hazard never visibly repeats. Player must time entry into each of the 4 pendulum columns then the door paddle. |
