...
|---------|------------|---------------------------|---------|
| v2.0.24 | 2026-05-16 | Corby Hoback · Claude Code | **Hard-tier expansion — 7 composed templates (issue #94 batch 2).** Closes the hard-pool gap (4 → 11) per the §4 grammar: `paddle-pair-phase`, `counter-patrols`, `channel-paddle`, `tight-pickets`, `triple-paddle`, `crossfire`, `gauntlet`. All amp the difficulty via walls-force-through-motion, stacked motions with non-aligned periods, or dense fast oscillator packing. No engine change. See §4 *Layout templates* (hard rows). |
...
**Current pool (35 templates — 11 simple + 4 easy + 7 medium + 11 hard + 2 expert as of v2.0.24):**
...
| `paddle-pair-phase` | Paddle pair (phase) | universal | hard | Two paddles 140 px apart in front of door, phase-offset π (one up while other down). Tight dash-through window; no "wait for both to clear" cheat. Period 2 s. |
| `counter-patrols` | Counter patrols | universal | hard | Top + bottom static walls compress player into a center corridor where two horizontal patrols sweep past each other on opposite y-bands in opposite directions. Period 2.4 s. Walls force player THROUGH the motion. |
| `channel-paddle` | Channel paddle | universal | hard | Static walls form a 160-px vertical channel on the right half; a paddle sweeps most of the channel inside. No way around — player commits to the channel + times the paddle. |
| `tight-pickets` | Tight pickets | universal | hard | Hard-tier upgrade of `pickets`: 5 narrow oscillators packed tighter (~90 px center-to-center, all right-half) and ~50% faster (period 1.5 s). Same 5-step phase wave. |
| `triple-paddle` | Triple paddle | universal | hard | Three paddles in a row in front of door, 120° phase-staggered — openings move as a rolling wave the player must read across three consecutive windows. Period 1.8 s. |
| `crossfire` | Crossfire | universal | hard | Three motions on three axes: horizontal patrol upper band + vertical pendulum middle + sine paddle at door. Non-aligned periods (2500/1800/1500) so the combined pattern doesn't visibly repeat. |
| `gauntlet` | Gauntlet | universal | hard | Three snake walls in the right half force the player through three narrow gaps; a paddle sits inside the middle gap, sweeping most of it. Already-constrained path + a hard timing call at the middle beat. |
...
- SWE simple = 10 (universal 7 + swe 3); SWE easy = 4 (universal); SWE medium = 7 (universal); SWE hard = 11 (universal); SWE expert = 2 (universal); SWE total eligible = 34.
- Homeschool simple = 8 (universal 7 + homeschool 1); Homeschool easy = 4 (universal); Homeschool medium = 7 (universal); Homeschool hard = 11 (universal); Homeschool expert = 2 (universal); Homeschool total eligible = 32.

In 2020 (100% simple) SWE picks from 10 / Homeschool from 8. From 2021 onwards the easy pool engages at the v2.0.9 mix weights. From 2023 onwards medium rolls pull from the seven medium templates rather than falling through to easy. From 2027 onwards hard rolls pull from the eleven hard templates. From 2029 onwards expert rolls pull `zigzag-sentinel` / `patrol-and-paddle` rather than falling through to hard.
