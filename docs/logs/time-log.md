...
| 4   | morning       | boundary | 2026-05-13T10:45:00Z | **product → process tooling boundary.** Wall time before this row = product dev (PRs #61, #62 — SWE icon coverage, preview generator, SWE text+icon previews, AI/human review research artifact). Wall time after this row = process tooling on Claude-side workflows: created universal `/research` skill at `~/.claude/skills/research/SKILL.md`; captured `context-estimation-bias-after-compact` research artifact; patched `/usage` skill with post-compaction correction; added project memory entries. Process tooling belongs to the meta-loop, not Path-to-the-Future product velocity — `session-process-log` should split the wall time on this boundary when computing time-savings or product throughput. |
